...
face_names = []
# Find location of faces in unkown image, encode those
unknown_face_locations = face_recognition.face_locations(unkown_image)
unkown_image_encoding = face_recognition.face_encodings(unkown_image, unknown_face_locations)
...
for (top, right, bottom, left), name in zip(unknown_face_locations, face_names):
  ...
  cv2.rectangle(unkown_image, (left, top), (right, bottom), (0, 255, 0), 2)

  # Draw label with name below corrosponding face
  cv2.rectangle(unkown_image, (left, bottom - 35), (right, bottom), (0, 255, 0), cv2.FILLED)
  font = cv2.FONT_HERSHEY_DUPLEX
  cv2.putText(unkown_image, name, (left + 6, bottom - 6), font, 1.0, (255, 255, 255), 1)

# Display image w/ bounding boxes over faces
cv2.imshow("image", unkown_image)
cv2.waitKey(0)
